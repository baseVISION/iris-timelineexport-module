...
def render_presentation(
    ...
    for slide_idx, chunk in enumerate(chunks, start=1):
        ...
        # Count unique days in this chunk — one day marker per day, always at least 1
        unique_days_in_chunk = len({ev["dt"].date() for ev in chunk})
        actual_items = len(chunk) + unique_days_in_chunk
        ...
        pos_idx = 0
        is_top = True
        current_day = None  # track day changes within the slide

        for ev_data in chunk:
            ev_day = ev_data["dt"].date()

            # Draw a day marker whenever the day changes (including first event)
            if ev_day != current_day:
                current_day = ev_day
                day_label = f"Day {day_num_map[ev_day]}"
                dx = x_origin + segment_width * pos_idx
                draw.rounded_rectangle(
                    [(dx - DAY_BOX_W // 2, CY - DAY_BOX_H // 2),
                     (dx + DAY_BOX_W // 2, CY + DAY_BOX_H // 2)],
                    radius=40, fill=C_DAY_BG)
                bb = f_day.getbbox(day_label)
                tw, th = bb[2] - bb[0], bb[3] - bb[1]
                draw.text((dx - tw // 2, CY - th // 2 - bb[1] - 1), day_label, font=f_day, fill=C_DAY_TEXT)
                pos_idx += 1
                is_top = True  # reset alternation on each new day
